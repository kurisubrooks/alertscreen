...
const getData = () => {
  return new Promise(resolve => {
    fetch('https://api.kurisubrooks.com/api/fire?raw').then(res => {
      res.json().then(data => {
        console.log(data.data);

        const allLayers = { type: 'FeatureCollection', features: [] };
        const emergencyLayer = { type: 'FeatureCollection', features: [] };
        const watchLayer = { type: 'FeatureCollection', features: [] };
        const adviceLayer = { type: 'FeatureCollection', features: [] };
        const noneLayer = { type: 'FeatureCollection', features: [] };

        data.data.features.forEach(element => {
          const level = element.properties.category;
...
map.on('mouseenter', 'fire-zone', () => {
  map.getCanvas().style.cursor = 'pointer';
});

map.on('mouseleave', 'fire-zone', () => {
  map.getCanvas().style.cursor = '';
});

const currentlyOpen = null;
const currentLayer = 'fires';
